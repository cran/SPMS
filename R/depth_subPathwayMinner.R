     ...
     for(p in 1:length(ruledata)){
	     for(i in 1:dim(ruledata[[p]])[1]){
		     if(ruledata[[p]][i,1]!=0){			     
				 ...
				 col4<-cbind(col4,ruledata[[p]][i,1],ruledata[[p]][i,2])				 
			     for(j in ruledata[[p]][i,1]:ruledata[[p]][i,2]){				     
     ...
     for(p in 1:length(ruledata)){
	     for(i in 1:(length(ruledata[[p]])/6)){
		     if(ruledata[[p]][i,1]!=0){
				 ...
				 col5<-cbind(col5,ruledata[[p]][i,3],(as.numeric(ruledata[[p]][i,2])-as.numeric(ruledata[[p]][i,1])+1),length(interestOfGene),g,ruledata[[p]][i,6])
			     subFunc<-gsub(" ","",paste("depth_subPathwayGraph(","\'",paths,"\'",",",(i+sum),",","\'",name,"\'",")"))
			     col5<-cbind(col5,subFunc)
				 for(j in ruledata[[p]][i,1]:ruledata[[p]][i,2]){
				     if(arrInG[j,p]!=0){
			             col5<-cbind(col5,as.character(arrInG[j,p]))
					 }
			     }				 
